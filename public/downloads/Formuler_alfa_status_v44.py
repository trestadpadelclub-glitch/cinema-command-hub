...
# Endast MyTVOnline3 (MOL3) räknas som "film" och triggar movie_*-scener.
# Andra appar (YouTube, Spotify, Red Bull TV, Netflix, VLC m.fl.) ska INTE
# påverka ljusautomatiken — användaren styr dem manuellt.
_FORMULER_PLAYER_PACKAGES = {
    # MyTVOnline3 (Formuler IPTV) — alla kända paketnamn
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_ir(self, action: str, body: Dict[str, Any]) -> None:
...
def main() -> None:
    ...
    _log(f"bridge ready  http://{addr[0]}:{addr[1]} (v44 + Broadlink IR)")
    ...
    if SETTINGS["broadlink_host"]:
        _log(f"broadlink     {SETTINGS['broadlink_host']}:{SETTINGS['broadlink_port']} mac={SETTINGS['broadlink_mac']} type=0x{SETTINGS['broadlink_devtype']:04x}")
    else:
