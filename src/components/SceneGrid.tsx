...
export function SceneGrid({ householdCode, activeSceneId }: Props) {
  ...
  const runScene = async (s: Scene) => {
    ...
    const results = await sendScene({
      ...
      marantzMute: s.marantz_mute,
      marantzSoundMode: s.marantz_sound_mode,
      marantzSmartSelect: s.marantz_smart_select,
      marantzDirac: s.marantz_dirac,
      marantzSpeakerPreset: s.marantz_speaker_preset,
      lightsOn: s.lights_on,
      sceneLights,
      projectorBlankDelaySeconds: s.projector_blank_delay_seconds,
